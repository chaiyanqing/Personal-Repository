(function(a,c){
	var b=function(e,d){
		...
		this.$element.on("click",".tree-folder-header .tree-folder-name, .tree-folder-header .icon-plus, .tree-folder-header .icon-minus",a.proxy(function(f){
			this.selectFolder(f.currentTarget)
		},this));
		this.$element.on("click",".addNode",a.proxy(function(f){
			this.options.addNode(JSON.parse(a(f.currentTarget).parent().context.id));
		},this));
		this.$element.on("click",".delNode",a.proxy(function(f){
			this.options.delNode(JSON.parse(a(f.currentTarget).parent().context.id));
		},this));
		this.$element.on("click",".updNode",a.proxy(function(f){
			this.options.updNode(JSON.parse(a(f.currentTarget).parent().context.id));
		},this));
		this.render();
	};
	b.prototype={
		constructor:b,render:function(){
			...
			this.options.dataSource.data(f.data(),function(g){
				...
				a.each(g.data,function(h,j){
					...
					if(j.type==="folder"){
						...
						i.find(".tree-folder-name").html("（"+h+"）" + j.name);
						i.find(".tree-folder-option").html("<span class='addNode "+e.options["addnode-icon"]+"' id='"+JSON.stringify(j)+"' title='添加'></span>");
						...
						if(j.type==="item"){
							...
							i.find(".tree-item-name").html("（"+h+"）" + j.name);
							i.find(".tree-item-option").html("<span class='delNode "+e.options["delnode-icon"]+"' id='"+JSON.stringify(j)+"')' title='删除'></span>");
							i.data(j);
			...
			if(this.options.multiSelect){
				a.each(g,function(i,j){var h=a(j);if(h[0]!==d[0]){f.push(a(j).data())}})
			}else{
				if(g[0]!==d[0]){
			...
			var e=this.$element.find(".tree-selected");var d=[];a.each(e,function(f,g){d.push(a(g).data())});return d
		}
	};
	...
	a.fn.tree.defaults={
		multiSelect:false,loadingHTML:"<div>Loading...</div>",cacheItems:true
	};
